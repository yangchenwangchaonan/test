...
type BaseConfig = EntitySharedConfig & AppearanceSharedConfig & NewEntity ;
...
> extends MushroomBaseElement {
  ...
  protected get _stateObj1(): E | undefined {
    if (!this._config || !this.hass || !this._config.entity_second) return undefined;

    const entityId = this._config.entity_second;
    return this.hass.states[entityId] as E;
  }
  ...
  protected renderStateInfo(
    ...
    const thirdly = stateObj1?.state && stateObj1?.attributes?.unit_of_measurement ? `${stateObj1.state}${stateObj1.attributes.unit_of_measurement}` : '';
    // computeInfoDisplay(
    //   appearance.secondary_info,
    //   name,
    //   displayState,
